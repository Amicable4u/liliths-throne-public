...
public class PlayerTalk {
	...
	public static final SexAction PLAYER_ASK = new SexAction(
		...
		public String getDescription() {
			...
			getAskResponse(asker, responder, pace);
		...
		public void applyEffects() {
			((NPC)Main.sex.getCharacterTargetedForSexAction(this)).generateSexChoices(
				true, Main.game.getPlayer(), Util.newArrayListOfValues(
					// TODO (mark): add option to begin suggested SexType
					new SexType(SexParticipantType.NORMAL, null, SexAreaOrifice.VAGINA)
				)
			);
		}
	};
	 ...
	 */
	private static void getAskResponse(GameCharacter asker, GameCharacter responder, SexPace sexPace) {
		...
		// SexType desired = responder.getMainSexPreference(asker);
		SexType desired = Util.getRandomObjectFromWeightedMap(preferences);
		SexAreaInterface performingArea = desired.getPerformingSexArea();
		SexAreaInterface targetArea = desired.getTargetedSexArea();
		String pname = performingArea.getName(responder);
		String tname = targetArea.getName(asker);
		...
		if (desired.isPenetrating()) {
			...
			switch (sexPace) {
				...
				case SUB_RESISTING:
					...
					break;
				default:
					UtilText.nodeContentSB.append("Unknown respond penetrating pace.<br/><br/>");
					break;
			}
      ...
      // CHOKE
			...
			String actionDescription = "my " + pname + " and your " + tname;
